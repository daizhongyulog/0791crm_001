...
public class UserService extends BaseService<User,Integer> {
    ...
    public UserModel userLogin(String userName,String userPwd){
       checkUserLoginParam(userName,userPwd);
        //用户是否存在
        User temp = userMapper.selectUserByNname(userName);
    ...
    private void checkPasswordParams(User user, String oldPassword, String newPassword, String confirmPwd) {
        ...
        //新密码不能和原始密码一致
        AssertUtil.isTrue(StringUtils.isBlank(oldPassword),"新密码和原始密码不能相同");
        //确认密码非空
    ...
    public void addUser(User user){
        //验证
        checkUser(user.getTrueName(),user.getEmail(),user.getPhone());
        //用户名唯一
        User temp = userMapper.selectUserByNname(user.getUserName());
    ...
    private void checkUser(String userName,String email,String phone){
        AssertUtil.isTrue(StringUtils.isBlank(userName),"用户名不为空");
    ...
    public void changeUser(User user){
        ...
        User temp2 = userMapper.selectUserByNname(user.getUserName());
        //修改用户名已存在问题
        AssertUtil.isTrue(temp2!=null && !(temp2.getId().equals(user.getId())),"用户名称已存在");
        //验证参数
